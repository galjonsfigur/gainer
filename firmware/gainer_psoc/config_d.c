		...
		if (inData == COMMAND_TERMINATOR) {
			...
			switch (*_gainer.cLocalRxBuffer) {
				case 'R':	// get all digital inputs (R)
					bNumBytes = config_d_command_get_din_all(_gainer.cLocalRxBuffer, FALSE);
					break;
/*				
				case 'r':	// get all digital inputs (continuous) (r)
					bNumBytes = config_d_command_get_din_all(_gainer.cLocalRxBuffer, TRUE);
					break;
*/
				case 'D':	// set all digital outputs (Dxxxx)
					bNumBytes = config_d_command_set_dout_all(_gainer.cLocalRxBuffer);
					break;
/*				
				case 'H':	// set the digital output high (Hn)
					bNumBytes = config_d_command_set_dout_ch_h(_gainer.cLocalRxBuffer);
					break;

				case 'L':	// set the digital output low (Ln)
					bNumBytes = config_d_command_set_dout_ch_l(_gainer.cLocalRxBuffer);
					break;
...
*/				
				case 'T':
					bNumBytes = config_d_command_set_sensitivity(_gainer.cLocalRxBuffer);
					break;
